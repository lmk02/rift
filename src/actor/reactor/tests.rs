...
fn shared_mode_reports_global_workspace_indices_to_clients() {
    ...
    assert!(
        active.is_some(),
        "workspace 7 should be the active one on its display"
    );
}
...
fn move_window_to_another_display_with_follow_activates_and_focuses_it() {
    ...
    assert!(
        !outcome.mouse_warps.is_empty(),
        "follow = true takes focus with the window"
    );
...
fn move_window_to_a_workspace_on_the_same_display_is_unchanged_by_shared_mode() {
    ...
    assert_eq!(assignment.workspace_id, target);
    assert!(reactor.state.windows.workspace_windows(right_space, target).is_empty());
}
...
fn an_unconfirmed_move_is_not_undone_by_a_snapshot_that_has_not_caught_up() {
    ...
    reactor.reassign_window_to_authoritative_space_preserving_workspace_ordinal(window, left_space);
    ...
    assert_eq!(
        assignment.workspace_id, workspace,
        "and it stays in its own workspace"
    );
}
